...
- 简要、标准、详细三档总结；方法角色与缺失字段；作者贡献和 Agent 推断分开呈现。耗时分析通过单工作线程后台队列执行，任务和结果写入 SQLite，页面显示状态并自动刷新，服务重启后可恢复已完成结果；相同活动任务会复用，排队中或运行中的任务可以取消。
...
$env:PAPER_AGENT_QA_TIMEOUT = "none" # 默认不限制；也可填写正数秒数
...
- GLM 限流、空响应或无效 JSON 会重试后明确失败；问答默认不设置客户端超时，也可通过 `PAPER_AGENT_QA_TIMEOUT` 配置正数秒数。
...
- 提供后台任务的阶段级进度推送，以及更细粒度的资源监控和模型调用级取消。
